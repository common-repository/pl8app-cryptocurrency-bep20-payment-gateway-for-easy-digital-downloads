...
var hideAddButton = function (cryptoId) {
	jQuery('#pl8app_edd_pro_redux_options-' + cryptoId + '_hd_mpk_sample_addresses .redux-multi-text-add').hide();
}
...
var generateMpkAddresses = function (cryptoId, numberOfSamples) {
	...
	xhr[cryptoId] = jQuery.ajax({
		...
		beforeSend: function () {
			...
			jQuery('#pl8app_edd_pro_redux_options-' + cryptoId + '_hd_mpk_sample_addresses input').removeClass('flash-red');
			jQuery('#pl8app_edd_pro_redux_options-' + cryptoId + '_hd_mpk_sample_addresses input').removeClass('flash-green');
			jQuery('#pl8app_edd_pro_redux_options-' + cryptoId + '_hd_mpk_sample_addresses input').addClass('flash-yellow');
		}
	}).fail(function(response) {
		jQuery('#pl8app_edd_pro_redux_options-' + cryptoId + '_hd_mpk_sample_addresses input').removeClass('flash-yellow');
		if (response.status === 0) {
			return;
		}
		updateSampleText(cryptoId, numberOfSamples, 'Address creation failed, please check your mpk.');

		jQuery('#pl8app_edd_pro_redux_options-' + cryptoId + '_hd_mpk_sample_addresses input').addClass('flash-red');

	}).done(function(responseJson) {
		jQuery('#pl8app_edd_pro_redux_options-' + cryptoId + '_hd_mpk_sample_addresses input').removeClass('flash-yellow');
		jQuery('#pl8app_edd_pro_redux_options-' + cryptoId + '_hd_mpk_sample_addresses input').addClass('flash-green');
...
jQuery(document).ready(function() {
	...
	hdCryptos.forEach(function (cryptoId) {
		...
		if (typeof this.getHdMode === 'function') {
			var hdModeKey = 'input[name="pl8app_edd_pro_redux_options[' + cryptoId + '_hd_mode]"]';
