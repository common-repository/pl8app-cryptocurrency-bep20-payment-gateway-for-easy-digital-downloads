...
class pl8app_edd_Carousel_Repo {
	private $tableName;
	public function __construct() {
		global $wpdb;
		$this->tableName = $wpdb->prefix . pl8app_edd_CAROUSEL_TABLE;

		$countCryptos = count(pl8app_edd_Cryptocurrencies::get());
	...
	public function set_index($cryptoId, $index) {
		global $wpdb;
		pl8app_edd_Util::log(__FILE__, __LINE__, 'Updating index for ' . $cryptoId . ' to ' . $index);
	...
	public function get_index($cryptoId) {
		...
		pl8app_edd_Util::log(__FILE__, __LINE__, 'Getting index: ' . $currentIndex);
		return $currentIndex;
	}

	public function set_buffer($cryptoId, $buffer) {
		global $wpdb;
		// pl8app_edd_Util::log(__FILE__, __LINE__, 'Updating buffer for ' . $cryptoId . ' to ' . print_r($buffer, true));

		$serializedBuffer = pl8app_edd_Util::serialize_buffer($buffer);
	...
	public function get_buffer($cryptoId) {
		...
		pl8app_edd_Util::log(__FILE__, __LINE__, 'Getting buffer: ' . print_r($result, true));
