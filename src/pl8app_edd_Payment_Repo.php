...
class pl8app_edd_Payment_Repo {
	private $tableName;

	public function __construct() {
		global $wpdb;

		$this->tableName = $wpdb->prefix . pl8app_edd_PAYMENT_TABLE;
	}

	public function insert($address, $cryptocurrency, $orderId, $paymentAmount, $status, $hdAddress = '0') {
		pl8app_edd_Util::log(__FILE__, __LINE__, 'inserting ' . $address . ' into db as ' . $status . ' with order amount of: ' . $paymentAmount);
		global $wpdb;
		$currentTime = time();
	...
	public function set_status($orderId, $orderAmount, $status) {
		global $wpdb;
		pl8app_edd_Util::log(__FILE__, __LINE__, 'updating ' . $orderId . ' to ' . $status);
