...
class pl8app_edd_Blockchain {
	...
	public static function get_blockchaininfo_total_received_for_btc_address($address, $requiredConfirmations) {
		...
		if (is_wp_error($response) || $response['response']['code'] !== 200) {
			pl8app_edd_Util::log(__FILE__, __LINE__, 'FAILED API CALL ( ' . $request . ' ): ' . print_r($response, true));
			$result = array (
				'result' => 'error',
				'total_received' => '',
			);
		...
		foreach ($rawTransactions as $rawTransaction) {
			...
			foreach ($outputs as $output) {
				if ($output[0] === $address) {
					...
					$transactions[] = new pl8app_edd_Transaction($amount,
														  10000,
		...
		foreach ($rawTransactions as $rawTransaction) {
			...
			foreach ($outputs as $output) {
				...
				if (in_array($addressToMatch, $output->scriptPubKey->addresses)) {
					...
					$transactions[] = new pl8app_edd_Transaction($amount, $confirmations, $timeStamp, $hash);
				}
			}
		}
	...
	public static function get_blk_address_transactions($address) {
		...
		if (property_exists($body, 'error')) {
			pl8app_edd_Util::log(__FILE__, __LINE__, 'FAILED API CALL ( ' . $request . ' ): ' . $body->error);
			$result = array(
				'result' => 'error',
				'total_received' => '',
			);
			...
			if ($rawTransactionId->type === 'vout' || $rawTransactionId->type === 'vin') {
				...
				foreach ($vouts as $vout) {
					if ($vout->scriptPubKey->addresses[0] === $address) {
						$transactions[] = new pl8app_edd_Transaction($vout->value * 100000000,
	...
	public static function get_bsv_address_transactions($address) {
		...
		foreach ($rawTransactions as $rawTransaction) {
			...
			$transactions[] = new pl8app_edd_Transaction($rawTransaction->value,
												  10000,
		...
		if (is_wp_error($response) || $response['response']['code'] !== 200) {
            ...
            foreach ($rawTransactions as $rawTransaction) {
                if ($rawTransaction->tx_input_n == -1) {
                    $transactions[] = new pl8app_edd_Transaction(
	...
	public static function get_doge_address_transactions($address) {
		...
		foreach ($rawTransactions as $rawTransaction) {
			...
			$transactions[] = new pl8app_edd_Transaction($rawTransaction->value * 100000000,
												  $rawTransaction->confirmations,
												  $rawTransaction->time,
												  $rawTransaction->txid);
	...
	public static function get_eos_address_transactions($address) {
		...
		foreach ($rawTransactions as $rawTransaction) {
			if ($rawTransaction->receiver === $address) {
				$transactions[] = new pl8app_edd_Transaction($rawTransaction->quantity * 10000,
													  10000,
	...
	public static function get_lsk_address_transactions($address) {
		...
		$transactions = array();
		foreach ($rawTransactions as $rawTransaction) {
			$transactions[] = new pl8app_edd_Transaction($rawTransaction->amount,
												  $rawTransaction->confirmations,
												  time(),
												  $rawTransaction->id);
	...
	public static function get_trx_address_transactions($address) {
		...
		foreach ($rawTransactions as $rawTransaction) {
			...
			if ($rawTransaction->toAddress === $address && $rawTransaction->confirmed) {
				$transactions[] = new pl8app_edd_Transaction($rawTransaction->contractData->amount,
													  10000,
	...
	public static function get_xem_address_transactions($address) {
		...
		$transactions = array();
		foreach ($rawTransactions as $rawTransaction) {
			$transactions[] = new pl8app_edd_Transaction($rawTransaction->transaction->amount,
												  10000,
												  time(),
		...
		foreach ($rawTransactions as $rawTransaction) {
			...
			if ($rawTransaction->type === 'create_account') {
				if ($rawTransaction->account === $address) {
					$transactions[] = new pl8app_edd_Transaction($rawTransaction->starting_balance * 10000000,
												  10000,
			...
			if ($rawTransaction->type === 'payment') {
				if ($rawTransaction->to === $address) {
					$transactions[] = new pl8app_edd_Transaction($rawTransaction->amount * 10000000,
												  10000,
												  strtotime($rawTransaction->created_at),
												  $rawTransaction->transaction_hash);
				}
	...
	public static function get_xrp_address_transactions($address) {
		...
		foreach ($rawTransactions as $rawTransaction) {
			if ($rawTransaction->tx->Destination === $address) {
				...
				$transactions[] = new pl8app_edd_Transaction($rawTransaction->tx->Amount,
												  10000,
	...
	public static function get_xtz_address_transactions($address) {
		...
		foreach ($rawTransactions as $rawTransaction) {
			if ($rawTransaction->account === $address && $rawTransaction->diff > 0) {
				...
				$transactions[] = new pl8app_edd_Transaction($rawTransaction->diff,
												  10000,
	...
	public static function get_erc20_address_transactions($contract_address, $address) {
		try{
			...
			$url = 'https://www.bscscan.com/token/generic-tokentxns2?m=normal&contractAddress='. $contract_address .'&a='. $address .'&sid='. trim($val) .'&p=1';
            ...
            if($rows) {
                ...
                foreach ($rows as $row) {
                    ...
                    if (!empty($timestamp) && !empty($hash)) {
                        $transactions[] = new pl8app_edd_Transaction($value, 10000, $timestamp, $hash);
                    }
                }
            }
