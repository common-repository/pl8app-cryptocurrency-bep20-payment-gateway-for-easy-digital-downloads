...
class pl8app_edd_Hd {
	public function __construct() {
		add_action('pl8app_edd_trigger_purchase_partial', array($this, 'trigger_purchase_partial'), NULL, 1);
	}
	...
	public static function buffer_ready_addresses($cryptoId, $mpk, $amount, $hdMode) {
		...
		for ($i = 0; $i < $neededAddresses; $i++) {
			...
			try {
				...
				pl8app_edd_Util::log(__FILE__, __LINE__, $e->getMessage());
			}
		}
	}
	...
	public static function check_all_pending_addresses_for_payment($cryptoId, $mpk, $requiredConfirmations, $percentToVerify, $hdMode) {
		...
		foreach ($pendingRecords as $record) {
			...
			if ($newPaymentAmount > 0.0000001) {
				...
				pl8app_edd_Util::log(__FILE__, __LINE__, 'Address ' . $address . ' received a new payment of ' . pl8app_edd_Cryptocurrencies::get_price_string($cryptoId, $newPaymentAmount) . ' ' . $cryptoId);
				...
				if ($paymentAmountVerified) {
					...
					$orderNote = sprintf(
						...
						pl8app_edd_Cryptocurrencies::get_price_string($cryptoId, $blockchainTotalReceived),
						$cryptoId,
				...
				// we received payment but it was not enough to meet store admin's processing requirement
					...
					if ($record['status'] === 'underpaid') {
						$orderNote = sprintf(
							...
							pl8app_edd_Cryptocurrencies::get_price_string($cryptoId, $newPaymentAmount),
							$cryptoId,
							pl8app_edd_Cryptocurrencies::get_price_string($cryptoId, ((float) $orderAmount) - $blockchainTotalReceived),
							$address);

						do_action('pl8app_edd_trigger_purchase_partial', $orderId);
					...
					// handle first underpayment, update status to pending payment (since we use on-hold for orders with no payment yet)
						...
						$orderNote = sprintf(
							...
							pl8app_edd_Cryptocurrencies::get_price_string($cryptoId, $amountToVerify - $blockchainTotalReceived),
							$address);
	...
	private static function get_total_received_for_bitcoin_address($address, $requiredConfirmations) {
		...
		$primaryResult = pl8app_edd_Blockchain::get_blockchaininfo_total_received_for_btc_address($address, $requiredConfirmations);

		if ($primaryResult['result'] === 'success') {
			return $primaryResult['total_received'];
		}

		$secondaryResult = pl8app_edd_Blockchain::get_blockexplorer_total_received_for_btc_address($address);

		if ($secondaryResult['result'] === 'success') {
			return $secondaryResult['total_received'];
		}

		$fallbackResult = pl8app_edd_Blockchain::get_chainso_total_received_for_btc_address($address);
	...
	private static function get_total_received_for_litecoin_address($address, $requiredConfirmations) {
		$primaryResult = pl8app_edd_Blockchain::get_blockcypher_total_received_for_ltc_address($address, $requiredConfirmations);

		if ($primaryResult['result'] === 'success') {
			return $primaryResult['total_received'];
		}

		$secondaryResult = pl8app_edd_Blockchain::get_chainso_total_received_for_ltc_address($address);
	...
	private static function get_total_received_for_qtum_address($address) {
		$result = pl8app_edd_Blockchain::get_qtuminfo_total_received_for_qtum_address($address);
	...
	private static function get_total_received_for_dash_address($address) {
		$result = pl8app_edd_Blockchain::get_dashblockexplorer_total_received_for_dash_address($address);
	...
	private static function get_total_received_for_doge_address($address) {
		$result = pl8app_edd_Blockchain::get_chainso_total_received_for_doge_address($address);
	...
	private static function get_total_received_for_xmy_address($address) {
		$result = pl8app_edd_Blockchain::get_blockbook_total_received_for_xmy_address($address);
	...
	private static function get_total_received_for_bitcore_address($address) {
		$result = pl8app_edd_Blockchain::get_insight_total_received_for_btx_address($address);
	...
	public static function cancel_expired_addresses($cryptoId, $mpk, $orderCancellationTimeSec, $hdMode) {
		...
		foreach ($assignedRecords as $record) {
			...
			pl8app_edd_Util::log(__FILE__, __LINE__, 'address ' . $address . ' has been assigned for ' . $assignedFor . '... cancel time: ' . $orderCancellationTimeSec);
			if ($assignedFor > $orderCancellationTimeSec && $totalReceived == 0) {
				...
				pl8app_edd_Util::log(__FILE__, __LINE__, 'Cancelled order: ' . $orderId . ' which was using address: ' . $address . 'due to non-payment.');
			}
		}
	}
	...
	private static function is_dirty_btc_address($address) {
		$primaryResult = pl8app_edd_Blockchain::get_blockchaininfo_total_received_for_btc_address($address, 0);
	...
	private static function is_dirty_ltc_address($address) {
		$primaryResult = pl8app_edd_Blockchain::get_chainso_total_received_for_ltc_address($address);
		...
		if ($primaryResult['result'] === 'success') {
			...
			if ($primaryResult['total_received'] >= 0.00000001) {
				...
				$secondaryResult = pl8app_edd_Blockchain::get_blockcypher_total_received_for_ltc_address($address, 0);
	...
	public static function force_new_address($cryptoId, $mpk, $hdMode) {
		...
		try {
			...
			pl8app_edd_Util::log(__FILE__, __LINE__, 'Could not create new addresses: ' . $e->getMessage());
		...
		try {
			if (!pl8app_edd_Util::p_enabled()) {
				...
				if (self::is_valid_mpk($cryptoId, $mpk)) {
					return apply_filters('pl8app_edd_get_hd_address', $cryptoId, $mpk, $index, $hdMode);
				}
			}
		}
		catch (\Exception $e) {
	...
	public function trigger_purchase_partial($payment_id) {
		...
		add_filter('edd_purchase_subject', 'pl8app_edd_change_partial_email_note_subject_line', 1, 2);
		add_filter('edd_purchase_heading', 'pl8app_edd_change_partial_email_heading', 1, 3);
