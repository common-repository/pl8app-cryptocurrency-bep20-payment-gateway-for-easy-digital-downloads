...
class pl8app_edd_Hd_Repo {
	...
	public function __construct($cryptoId, $mpk, $hdMode) {
		...
		$this->hdMode = $hdMode;
		$this->tableName = $wpdb->prefix . pl8app_edd_HD_TABLE;
	}

	public function insert($address, $mpk_index, $status) {
		pl8app_edd_Util::log(__FILE__, __LINE__, 'inserting ' . $address . ' into db as ' . $status);
		global $wpdb;
		$currentTime = time();
	...
	public function get_oldest_ready() {
		...
		pl8app_edd_Util::log(__FILE__, __LINE__, "Oldest ready address is: " . print_r($address, true));
		return $address;
	}
	...
	public function set_total_received($address, $totalReceived) {
		global $wpdb;
		pl8app_edd_Util::log(__FILE__, __LINE__, 'Updating total received at ' . $address .' to: ' . $totalReceived);
	...
	public function set_order_amount($address, $orderAmount) {
		global $wpdb;
		pl8app_edd_Util::log(__FILE__, __LINE__, 'Updating order amount at ' . $address . ' to: ' . $orderAmount);
	...
	public function set_status($address, $status) {
		global $wpdb;
		pl8app_edd_Util::log(__FILE__, __LINE__, 'Updating ' . $address . ' to ' . $status);
		if ($status === 'assigned') {
			$currentTime = time();
	...
	public function set_order_id($address, $orderId) {
		global $wpdb;
		pl8app_edd_Util::log(__FILE__, __LINE__, 'Setting address ' . $address . ' order id to: ' . $orderId);
