...
class pl8app_edd_Validation {
	public static function validate_redux_options($newValues, $oldValues) {
		$oldSettings = new pl8app_edd_Settings($oldValues);
		$newSettings = new pl8app_edd_Settings($newValues);
		...
		foreach (pl8app_edd_Cryptocurrencies::get() as $crypto) {
			...
			if ($newSettings->basic_enabled($cryptoId) || $newSettings->autopay_enabled($cryptoId)) {
				...
				foreach ($addresses as $ind => $address) {
					if (pl8app_edd_Cryptocurrencies::is_valid_wallet_address($cryptoId, $address)) {
				...
				if (pl8app_edd_Util::p_enabled()) {
					...
					if (pl8app_edd_Hd::is_valid_ypub($mpk) || pl8app_edd_Hd::is_valid_zpub($mpk)) {
						$invalidCryptoSettings = true;
						$atLeastOneInvalidCrypto = true;
						if (pl8app_edd_Hd::is_valid_mpk($cryptoId, $mpk)) {
						...
						if (!pl8app_edd_Hd::is_valid_xpub($mpk)) {
							$invalidCryptoSettings = true;
							$atLeastOneInvalidCrypto = true;
							$errorMessages[] = $cryptoName . ' has an invalid HD MPK. Disabling ' . $cryptoName . '.';
						}
			...
			if (pl8app_edd_Util::p_enabled()) {
				if (!pl8app_edd_Hd::is_valid_mpk($cryptoId, $newSettings->get_mpk($cryptoId))) {
					unset($newValues[$cryptoId . '_hd_mpk']);
				}
			}
			else {
				if (!pl8app_edd_Hd::is_valid_xpub($newSettings->get_mpk($cryptoId))) {
					unset($newValues[$cryptoId . '_hd_mpk']);
				}
			}
		...
		$reduxInstance = ReduxFrameworkInstances::get_instance(pl8app_edd_REDUX_ID);
