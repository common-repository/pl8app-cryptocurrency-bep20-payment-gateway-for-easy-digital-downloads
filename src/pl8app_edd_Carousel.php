...
class pl8app_edd_Carousel {
	...
	public function get_next_address() {
		...
		while (!pl8app_edd_Cryptocurrencies::is_valid_wallet_address($this->cryptoId, $nextAddress)) {
	...
	private function increment_current_index($cryptoId) {
		...
		$reduxOptions = get_option(pl8app_edd_REDUX_ID);
		...
		elseif ($this->currentIndex == ($seatCount - 1)) {
			...
			pl8app_edd_Util::log(__FILE__, __LINE__, 'Invalid current index! Something went wrong, please contact plug-in support.');
		}
	}
}
