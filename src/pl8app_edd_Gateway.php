...
class pl8app_edd_Gateway {
    ...
    public function __construct() {
        ...
        $cryptoArray = pl8app_edd_Cryptocurrencies::get();
        ...
        $this->id = 'pl8app_edd_pro_gateway';
        $this->title = $pl8appEddSettings->get_customer_gateway_message();
        $this->has_fields = true;
        $this->method_title = 'pl8app Crypto Payments';
        $this->method_description = 'Allow customers to pay using cryptocurrency';

        add_filter('edd_purchase_receipt', array($this, 'additional_email_details'), NULL, 3);

        add_action('wp_enqueue_scripts', array($this, 'thank_you_register_script'));

        add_action('edd_pl8app_cc_form', array($this, 'payment_fields'));
        add_action('edd_gateway_pl8app', array($this, 'process_payment'));
        add_action('edd_payment_receipt_after_table', array($this, 'thank_you_page'));
        // Emails are fired after we process the purchase, so hook additional email details here
        add_action('pl8app_edd_trigger_purchase_receipt', array($this, 'trigger_purchase_receipt'), NULL, 1);
    }
    ...
    public function payment_fields() {
        ...
        <fieldset id="edd_cc_fields" class="edd-do-validate">
            ...
            <?php endif; ?>
            <p id="pl8app-edd-cryptocurrency-wrap">
                <label for="pl8app_edd_currency_id" class="edd-label">
                    <?php echo 'Choose a cryptocurrency'; ?>
                    <span class="edd-required-indicator">*</span>
                    <span class="card-type"></span>
                </label>
                <select name="pl8app_edd_currency_id" id="pl8app_edd_currency_id" data-nonce="<?php echo wp_create_nonce( 'edd-pl8app-currency-id-field-nonce' ); ?>" class="pl8app_edd_currency_id edd-select<?php if( edd_field_is_required( 'pl8app_edd_currency_id' ) ) { echo ' required'; } ?>"<?php if( edd_field_is_required( 'pl8app_edd_currency_id' ) ) {  echo ' required '; } ?>>
                    <?php

                    $pl8appEddSettings = new pl8app_edd_Settings(get_option(pl8app_edd_REDUX_ID));

                    $validCryptos = $pl8appEddSettings->get_valid_selected_cryptos();
                    ...
                    foreach ($validCryptos as $crypto) {
                        ...
                        if ($pl8appEddSettings->hd_enabled($cryptoId)) {
                            ...
                            $hdRepo = new pl8app_edd_Hd_Repo($cryptoId, $mpk, $hdMode);

                            $count = $hdRepo->count_ready();

                            if ($count < 1) {
                                try {
                                    pl8app_edd_Hd::force_new_address($cryptoId, $mpk, $hdMode);
                                }
                                catch ( \Exception $e) {
                                    pl8app_edd_Util::log(__FILE__, __LINE__, 'UNABLE TO GENERATE HD ADDRESS FOR ' . $crypto->get_name() . ' ADMIN MUST BE NOTIFIED. REMOVING CRYPTO FROM PAYMENT OPTIONS' . $e->getTraceAsString());
    ...
    public function process_payment($purchase_data) {
        // handle different EDD currencies and get the order total in USD
        $curr = edd_get_currency();
        ...
        $selectedCryptoId = sanitize_text_field($purchase_data['post_data']['pl8app_edd_currency_id']);
        EDD()->session->set('chosen_crypto_id', $selectedCryptoId);
        $crypto = $this->cryptos[$selectedCryptoId];
        $cryptoId = $crypto->get_id();

        edd_update_payment_meta($payment_id, '_edd_pl8app_crypto_type_id', $cryptoId);

        $cryptoMarkupPercent = $pl8appEddSettings->get_markup($cryptoId);
        ...
        $bnb_flat_price = pl8app_edd_Exchange::get_bnb_flat_price($curr, 60);

        $pl8app_bnb_rate = pl8app_edd_Exchange::get_pl8app_bnb_price($cryptoId, $crypto->get_update_interval());
        ...
        $formattedCryptoTotal = pl8app_edd_Cryptocurrencies::get_price_string($cryptoId, $cryptoTotal);

        edd_update_payment_meta($payment_id, '_edd_pl8app_crypto_amount', $formattedCryptoTotal);

        pl8app_edd_Util::log(__FILE__, __LINE__, 'Crypto total: ' . $cryptoTotal . ' Formatted Total: ' . $formattedCryptoTotal);
        ...
        if ($pl8appEddSettings->hd_enabled($cryptoId)) {
            ...
            EDD()->session->set('hd_wallet_address', $orderWalletAddress);
        ...
        // HD is not enabled, just handle static wallet or carousel mode
            ...
            $orderWalletAddress = $pl8appEddSettings->get_next_carousel_address($cryptoId);

            // handle payment verification feature
            if ($pl8appEddSettings->autopay_enabled($cryptoId)) {
        ...
        // For email
        EDD()->session->set($cryptoId . '_amount', $formattedCryptoTotal);

        // For customer reference and to handle refresh of thank you page
        edd_update_payment_meta($payment_id, '_edd_pl8app_wallet_address', $orderWalletAddress);

        do_action('pl8app_edd_trigger_purchase_receipt', $payment_id);
    ...
    public function thank_you_register_script() {
        wp_register_script(
            'pl8app-edd-thank-you-page',
            pl8app_edd_PLUGIN_DIR . '/assets/js/pl8app-edd-thank-you-page.js',
            ['jquery'],
            NULL,
            true
        );
        ...
        wp_register_script(
            'web3',
            pl8app_edd_PLUGIN_DIR . '/assets/js/web3.min.js',
            NULL,
            NULL,
            true
        );
        ...
        wp_register_script(
            'bnblib',
            pl8app_edd_PLUGIN_DIR . '/assets/js/bnblib.js',
        ...
        wp_register_script(
            'pl8app-edd-pay-with-binance',
            pl8app_edd_PLUGIN_DIR . '/assets/js/pl8app-edd-pay-with-binance.js',
            ['bnblib'],
            NULL,
            true
        );
        ...
        wp_register_script(
            'pl8app-edd-pay-with-metamask',
            pl8app_edd_PLUGIN_DIR . '/assets/js/pl8app-edd-pay-with-metamask.js',
            ['bnblib'],
            NULL,
            true
        );
    ...
    public function thank_you_page() {
        ...
        $cssPath = pl8app_edd_PLUGIN_DIR . '/assets/css/pl8app-edd-thank-you-page.css';
        $fontcssPath = pl8app_edd_PLUGIN_DIR . '/assets/css/all.min.css';
        wp_enqueue_style('wp-jquery-ui-dialog');
        wp_enqueue_style('pl8app-edd-styles', $cssPath);
        wp_enqueue_style('pl8app-edd-fontawesome-styles', $fontcssPath);

        wp_enqueue_script('pl8app-edd-thank-you-page');
        wp_enqueue_script('pl8app-edd-pay-with-binance');
        wp_enqueue_script('pl8app-edd-pay-with-metamask');

        $payment = get_post($edd_receipt_args['id']);

        $cryptoId = edd_get_payment_meta($payment->ID, '_edd_pl8app_crypto_type_id', true);
        $crypto = $this->cryptos[$cryptoId];

        $orderWalletAddress = edd_get_payment_meta($payment->ID, '_edd_pl8app_wallet_address', true);

        $formattedCryptoTotal = edd_get_payment_meta($payment->ID, '_edd_pl8app_crypto_amount', true);
        ...
        try {
            ...
            edd_insert_payment_note($payment->ID, $e->getMessage());

            pl8app_edd_Util::log(__FILE__, __LINE__, 'Something went wrong during checkout: ' . $e->getMessage());
    ...
    public function additional_email_details($email_body, $payment_id, $payment_data) {
        ...
        if ('abandoned' === edd_get_payment_status($payment_id)) {
            ob_start(); ?>
            <p>Dear {name},</p>
            ...
            return ob_get_clean();
        } else if ('pending' === edd_get_payment_status($payment_id)) {
            $chosenCrypto = EDD()->session->get('chosen_crypto_id');
            $crypto =  $this->cryptos[$chosenCrypto];
            $orderCryptoTotal = EDD()->session->get($crypto->get_id() . '_amount');
            $orderWalletAddress = edd_get_payment_meta($payment->ID, '_edd_pl8app_wallet_address', true);
            $contract_address = pl8app_edd_Cryptocurrencies::get_erc20_contract($crypto->get_id());
            ...
            <p>
                ...
                <?php
                    if ($crypto->get_symbol() === '') {
                        echo esc_html(pl8app_edd_Cryptocurrencies::get_price_string($crypto->get_id(), $orderCryptoTotal) . ' ' . $crypto->get_id());
                    }
                    else {
                        echo esc_html($crypto->get_symbol() . pl8app_edd_Cryptocurrencies::get_price_string($crypto->get_id(), $orderCryptoTotal));
                    }
                ?>
            </p>
    ...
    private function get_select_options_for_valid_cryptos() {
        ...
        foreach (pl8app_edd_Cryptocurrencies::get_alpha() as $crypto) {
    ...
    private function get_qr_code($crypto, $walletAddress, $cryptoTotal) {
        $dirWrite = pl8app_edd_ABS_PATH . '/assets/img/';
        ...
        try {
            ...
            pl8app_edd_Util::log(__FILE__, __LINE__, 'QR code generation failed, falling back...');
            $endpoint = 'https://api.qrserver.com/v1/create-qr-code/?data=';
            return $endpoint . $qrData;
        }
        $dirRead = pl8app_edd_PLUGIN_DIR . '/assets/img/';
        return $dirRead . 'tmp_qrcode.png';
    }

    private function output_thank_you_html($crypto, $orderWalletAddress, $cryptoTotal, $orderId) {
        $formattedPrice = pl8app_edd_Cryptocurrencies::get_price_string($crypto->get_id(), $cryptoTotal);
        $pl8appEddSettings = new pl8app_edd_Settings(get_option(pl8app_edd_REDUX_ID));

        $customerMessage = apply_filters('pl8app_edd_customer_message', $pl8appEddSettings->get_customer_payment_message($crypto), $crypto, $orderId, $formattedPrice, $orderWalletAddress);
        $contract_address = pl8app_edd_Cryptocurrencies::get_erc20_contract($crypto->get_id());
        ...
        <ul class="order_details">
            <li>
                ...
                <div class="Pay-binance-button hidden">
                    <img src="<?php print(pl8app_edd_PLUGIN_DIR . '/assets/img/bsc-icon-logo-1-1.svg'); ?>" />
        ...
        <div id="dialog" title="Pay with Binance Wallet">
            ...
            <div class="dialog-step dialog-step-2 hidden">
                ...
                <input class="edd-submit blue button disabled" type="submit" value="Next">
            </div>
            ...
            <div class="dialog-step dialog-step-4 hidden">
                ...
                <input class="edd-submit blue button" type="submit" value="Close">
            </div>
            <div class="dialog-step dialog-step-5 hidden">
                <p class="edd-alert edd-alert-error">The following error occurred during payment:</p>
    ...
    private function get_crypto_value_in_usd($cryptoId, $updateInterval) {
        ...
        $prices = array();
        $reduxSettings = get_option(pl8app_edd_REDUX_ID);
        ...
        if (in_array('0', $selectedPriceApis)) {
            $ccPrice = pl8app_edd_Exchange::get_cryptocompare_price($cryptoId, $updateInterval);
        ...
        if (in_array('1', $selectedPriceApis)) {
            $hitbtcPrice = pl8app_edd_Exchange::get_hitbtc_price($cryptoId, $updateInterval);
        ...
        if (in_array('2', $selectedPriceApis)) {
            $gateioPrice = pl8app_edd_Exchange::get_gateio_price($cryptoId, $updateInterval);
        ...
        if (in_array('3', $selectedPriceApis)) {
            $bittrexPrice = pl8app_edd_Exchange::get_bittrex_price($cryptoId, $updateInterval);
        ...
        if (in_array('4', $selectedPriceApis)) {
            $poloniexPrice = pl8app_edd_Exchange::get_poloniex_price($cryptoId, $updateInterval);
...
/**
 * Load pl8app_edd_Gateway
 *
 * @return object pl8app_edd_Gateway
 */
function pl8app_edd() {
	return new pl8app_edd_Gateway();
}
